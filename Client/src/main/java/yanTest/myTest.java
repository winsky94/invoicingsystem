...
//库存赠送单
		GiftController controller = null;
		try {
		...
		ArrayList<GiftVO> giftList=controller.getGiftList();
		try {
			GiftReceipt receipt=new GiftReceipt();
			int result=receipt.excute(giftList.get(0));
			System.out.println(result);
		} catch (Exception e) {
			// TODO 自动生成的 catch 块
			e.printStackTrace();
		}
		giftList=controller.getGiftList();
		for(int i=0;i<giftList.size();i++){
			System.out.println(giftList.get(i).getGiftList().get(0).getName()+":"+giftList.get(i).getStatus());
		}
		
		
		
//库存报损单的审批通过并执行-----------------------------------------------------
//		StockControlController controller=new StockControlController();
//		ArrayList<StockOverOrLowVO> slist=controller.showStockLowReceipt();
//		for(int i=0;i<slist.size();i++){
//			System.out.println(slist.get(i).getId()+":"+slist.get(i).getGoodsName());
//		}
//		try {
//			StockLowReceipt receipt=new StockLowReceipt();
//			receipt.excute(slist.get(0));
//		} catch (Exception e) {
//			// TODO 自动生成的 catch 块
//			e.printStackTrace();
//		}
...
//		
		...
		for (int i = 0; i < list.size(); i++) {
			System.out.println(list.get(i).getName()+":"+list.get(i).getNumInStock());
		}
