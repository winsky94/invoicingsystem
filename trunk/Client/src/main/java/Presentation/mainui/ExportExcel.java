...
public class ExportExcel{
	...
	//包含表头在内的二维结构
	public static void Exprot(ArrayList<ArrayList<String>> s,String fileName){
		...
		try {//"f://test2.xls"
			...
			sb.append(" </Styles>\n");
			int sheetcount = 0;
			int recordcount = s.size();//一个sheet里规定的行数 可做拓展
			int currentRecord = 0;
			int total =s.size();
			int col = s.get(0).size();
			...
			for (int i = 0; i < total; i++) {
				...
				for (int j = 0; j < col; j++) {
					...
					sb.append("<Cell><Data ss:Type=\"String\">"+s.get(i).get(j)+"</Data></Cell>");
					sb.append("\n");
				}
				sb.append("</Row>");
				if (i % 5000 == 0) {
